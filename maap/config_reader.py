...
class ConfigReader(metaclass=Singleton):
    def __init__(self):
        ...
        config_paths = list(map(self.__get_config_path, [os.curdir, os.path.expanduser("~"), os.environ.get("MAAP_CONF") or '.']))
        ...
        self.__search_granule_url = self.__config.get("service", "search_granule_url")
        self.__search_collection_url = self.__config.get("service", "search_collection_url")
        self.__algorithm_register = self.__config.get("service", "algorithm_register")
        self.__algorithm_build = self.__config.get("service", "algorithm_build")
        self.__mas_algo = self.__config.get("service", "mas_algo")
        self.__dps_job = self.__config.get("service", "dps_job")
        self.__wmts = self.__config.get("service", "wmts")
        self.__member = self.__config.get("service", "member")
        self.__tiler_endpoint = self.__config.get("service", "tiler_endpoint")
        self.__maap_host = self.__config.get("service", "maap_host")
        self.__query_endpoint = self.__config.get("service", "query_endpoint")
    ...
    @property
    def search_granule_url(self):
        return self.__search_granule_url

    @search_granule_url.setter
    def search_granule_url(self, val):
        """
        :param val:
        :return: None
        """
        self.__search_granule_url = val
        return

    @property
    def search_collection_url(self):
        return self.__search_collection_url

    @search_collection_url.setter
    def search_collection_url(self, val):
        """
        :param val:
        :return: None
        """
        self.__search_collection_url = val
        return
